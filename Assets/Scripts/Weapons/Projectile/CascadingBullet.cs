...
public class CascadingBullet : Bullet
{
    ...
    protected override void PerformHitCheck(Vector3 startLoc, bool showTrailTracerVisuals)
    {
        ...
        foreach (var target in hitObj)
        {
            ...
            foreach (var cascadeHit in cascadeHits)
            {
                    ...
                    !HasHit(cascadeHit.gameObject))
                {
                    ...
                    var trailObj = ObjectPooler.GetObject(GetName() + "Trail", bulletTrail, target.transform.position, Quaternion.identity);
